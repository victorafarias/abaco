...
O método `carregarFuncoesDistintas()` é chamado durante a edição de um sistema:

- **Linha 144**: Chamado no `ngOnInit()` ao carregar um sistema existente
...
Método `getFuncoesDistintas()` - **PRINCIPAL GARGALO**:
...
**Problemas**:
- `@EntityGraph` carrega **todos** os relacionamentos mesmo quando apenas `nome`, `funcionalidade` e `modulo` são necessários
...
- Cada `FuncaoDados` carrega: funcionalidade + módulo + rlrs[] + ders[] + fatorAjuste
- Cada `FuncaoTransacao` carrega: funcionalidade + módulo + alrs[] + ders[] + fatorAjuste
...
Baseado na conversa anterior (ID: `44b562b0-2309-4149-aadf-84e403d35074`), foi implementado recentemente um loader na tela de edição de sistemas que permanece visível durante todo o carregamento das funções distintas, incluindo a chamada ao método `carregarFuncoesDistintas()` (linha 144 do componente).

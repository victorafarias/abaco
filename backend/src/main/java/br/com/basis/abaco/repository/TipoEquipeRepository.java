...
public interface TipoEquipeRepository extends JpaRepository<TipoEquipe, Long> {
    ...
    @Query("select t from TipoEquipe t join t.usuarios u join t.organizacoes o where u.login=:login and o.id=:idOrganizacao order by t.nome")
    ...
    @Query(value = "SELECT e.* FROM {h-schema}tipo_equipe e LEFT JOIN {h-schema}tipoequipe_organizacao eo ON e.id = eo.tipoequipe_id WHERE organizacao_id = :idOrg AND e.id NOT IN (SELECT ac.equipe_id FROM {h-schema}analise_compartilhada ac WHERE analise_id = :idAnalise) AND e.id != :idEquipe" ,nativeQuery = true)

...
export class VisaoPfComponent implements OnInit {
    ...
    @Output() finalizacaoContagem: EventEmitter<any> = new EventEmitter();
    ...
    validacaoContinuar(){
        ...
        for(var tela of this.visaopf.cenario.telasResult){
            if(!tela.dataUrlResult){
                ...
                valido = false
                this.showErrorMsg(`É necessário inserir o tipo de tela na imagem ${tela.originalImageName.split("@")[1] } `)
                break
            }
        }
        if(valido){
    ...
    continuarContagem(){
        if(this.validacaoContinuar()){ 
            this.finalizacaoContagem.emit(this.visaopf);
        }
    }
    ...
    showSucessMsg(msg):void{
        this.msgs = []
        this.msgs.push({ severity: 'success', summary: 'Sucesso ', detail: msg })
    }
